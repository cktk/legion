...
public class LegionVueGenerator {
    ...
    public String generate(String template, boolean api, boolean isDrawer, boolean enableTable, String vueName, Integer rowNum, List<Field> fields) throws IOException {
        ...
        if ("tree.btl".equals(template)) {
            if (!containsField(fields, "legionTreeTitle")) {
                fields.add(0, new Field().setType("legionTreeTitle").setSortOrder(new BigDecimal("-1")).setEditable(true));
            }
            if (!containsField(fields, "legionTreeChoose")) {
                fields.add(0, new Field().setType("legionTreeChoose").setSortOrder(new BigDecimal("-2")).setEditable(true));
            }
            if (!containsField(fields, "legionTreeSortOrder")) {
                fields.add(new Field().setType("legionTreeSortOrder").setSortOrder(new BigDecimal("999")).setEditable(true));
            }
        }
        ...
        if (rowNum == 1) {
            ...
            throw new LegionException("rowNum仅支持数字1-4");
        }
        // 生成代码
