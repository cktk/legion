...
import com.esmooc.legion.activiti.service.ActStarterService;
...
public class ActProcessServiceImpl implements ActProcessService {
    ...
    public String startProcess(ActBusiness actBusiness) {
        ...
        if (actProcess.getAllUser() != null && !actProcess.getAllUser()) {
            ...
            if (!actStarterService.hasRecord(actBusiness.getProcDefId(), userId)) {
                throw new LegionException("抱歉，您无权发起该流程");
            }
        }
        // 启动流程用户
        ...
        for (Task task : tasks) {
            if (actBusiness.getFirstGateway()) {
                ...
                if (users == null || users.isEmpty()) {
                    throw new LegionException("有任务节点未分配任何候选审批人，发起流程失败");
                } else {
                    // 分配了节点负责人分发给全部
                    for (User user : users) {
                        ...
                        messageUtil.sendActMessage(user.getId(), ActivitiConstant.MESSAGE_TODO_ID, actBusiness,
                                ActivitiConstant.MESSAGE_TODO_CONTENT, actBusiness.getSendMessage(),
    ...
    public ProcessNodeVo getFirstNode(String procDefId) {
        ...
        FlowElement e = null;
        if (startEvent == null) {
            throw new LegionException("未找到该流程起始节点，请确认流程设计是否正确");
        }
        // 判断开始后的流向节点
        ...
        for (FlowElement element : elements) {
            if (element.getId().equals(sequenceFlow.getTargetRef())) {
                if (element instanceof UserTask) {
                    ...
                    throw new LegionException("流程设计错误，开始节点后只能是用户任务节点、排他网关、并行网关");
                }
            }
        }
        if (e == null) {
            throw new LegionException("未找到该流程起始后的任何节点，请确认流程设计是否正确");
        }
        // 排他、平行网关直接返回
        ...
        for (ActivityImpl activityImpl : activitiList) {
            if (activityImpl.getId().equals(currActId)) {
                ...
                if ("userTask".equals(type)) {
                    ...
                    throw new LegionException("流程设计错误，包含无法处理的节点");
                }
                break;
            }
        }
    ...
    public ProcessNodeVo getNode(String nodeId, String taskId, String hisTaskId) {
        ...
        if (actNodeService.hasCustomUser(nodeId)) {
            ...
            if (StrUtil.isBlank(taskId)) {
                throw new LegionException("自选用户节点taskId不能为空");
            }
            ...
            if (StrUtil.isBlank(hisTaskId)) {
                throw new LegionException("连续部门负责人类型节点hisTaskId不能为空");
            }
            // 如果是连续部门负责人类型
