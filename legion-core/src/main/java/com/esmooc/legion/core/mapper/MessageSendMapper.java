...
public interface MessageSendMapper extends BaseMapper<MessageSend> {
     ...
     * @param status
     */
    @Select("update t_message_send m set m.status=#{status} where m.user_id=#{userId}")
     ...
     * @param status
     */
    @Select("delete from t_message_send m where m.user_id = #{userId} and m.status=#{status}")
